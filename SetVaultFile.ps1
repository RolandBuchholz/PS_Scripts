<# .SYNOPSIS
     Upload von Dateien in den Workspace 
.DESCRIPTION
...
.VERSION
     Version 0.3 – Testversion Errorcode handling hizugefügt
.EXAMPLE
    ...
    for ($i=0; $i -le $uploadFiles.Count-1; $i++){
        ...
        switch ($uploadFile._Extension)
        {
          ...
          "html" {
                ...
                $motortyp = ($HTML.body.getElementsByTagName('tr') | Where {$_.innerText -like "Motortyp*"}).innerText
                $aufhaengung=($HTML.body.getElementsByTagName('tr') | Where {$_.innerText -like "Aufhängung*"}).innerText.Replace("Aufhängung is ","")
                $lageTreibscheibe =($HTML.body.getElementsByTagName('tr') | Where {$_.innerText -like "Treibscheibe *"}).innerText[0]
                $treibscheibe =($HTML.body.getElementsByTagName('tr') | Where {$_.innerText -like "Treibscheibe *"}).innerText[2]
